...
def codeName_extraction(path, name, list_busCode, list_busCode_original):
    ...
    with open(path + '/' + name) as f:
        for line in f:
            ...
            list_index = match_road(split_line[1].lower(), list_busCode) #we can also use the function in extract road name in extract the bus code name
            if (len(list_index) > 0):
                for index in list_index:
                    print (split_line[0] + '\t' + list_busCode_original[index + 1]) #list_busCode_original have different index with list_busCode;
